...
# Convencion estructural del ecosistema: el paso que redacta lo que se le dice al
# usuario se titula "Informar al usuario" (42 SKILL.md la siguen). Todo lo que cae
# bajo ese encabezado ES, por construccion, mensaje al usuario.
USER_REPORT_HEADING_RE = re.compile(
    r"^#{2,6}\s.*\b(informar|informe|informa|reportar|comunicar|mensaje|salida)\b"
...
def check_user_facing_commands(findings):
    """USER-FACING-COMMAND — un mensaje dictado al usuario que contiene un `wf-*`.
       ...
       Dos detectores, ambos de FORMA (nunca de semantica):

       (a) Mensaje dictado: una linea de BLOCKQUOTE que ABRE con comillas (`> "..."`),
           que es como los SKILL.md dictan lo que hay que decirle al usuario.
       (b) Seccion de informe: cualquier linea bajo un encabezado "Informar al usuario"
           (y variantes). Es la convencion estructural del ecosistema para el paso que
           redacta la salida, asi que ahi dentro TODO es mensaje al usuario por
           construccion — incluida la forma inline que (a) no ve: "Informa: … siguientes
           pasos `/wf-x <args>`". Ese hueco costo 19 nombres de workflow filtrados a los
           artefactos de una corrida real (pasada 8 de CU-3.a).

       La prosa dirigida al agente, las citas de decision y los prompts de delegacion
       —que DEBEN nombrar el `SKILL.md` del delegado— no caen en ninguno de los dos:
       van sin comillas y fuera de la seccion de informe.
    ...
    for skill_md in sorted(SDD_ROOT.rglob("SKILL.md")):
        ...
        for off, line in enumerate(body.splitlines()):
            ...
            if not is_user_surface:
                ...
                # comillas por dentro (p. ej. un argumento shell entrecomillado a
                # mitad de frase).
