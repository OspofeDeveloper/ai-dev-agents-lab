...
> "Uso: `/spec-decompose <archivo_spec.md>`"
> "Ejemplo: `/spec-decompose docs/proyecto_spec.md`"
...
2. Verifica que el nombre termina en `_spec.md`. Si no → informa:
   > "Este archivo no parece un Spec procesado. Primero genera el spec con `/spec-finalize <archivo.md>`"
...
5. Verifica que contiene evidencia de Spec SDD válido (presencia de "Criterios de Aceptación" e "Historias de Usuario"). Si no → informa:
   > "Este archivo no parece un Spec SDD procesado. Primero ejecuta `/spec-finalize <archivo.md>`"

---

## Paso 3: Identificar y validar features

Usando las reglas de `decompose-expert` (cohesión funcional, no secciones del documento):
...
2. **Si hay modelos sin owner definitivo:**
   - Presenta la tabla al usuario con los modelos ambiguos:
     ...
     ```
   - Para cada modelo, aplica el criterio de desempate de `decompose-expert` y explica al usuario cuál candidato sale ganador y por qué
...
Si se generaron **2 o más features**, ejecuta el workflow `spec-conflict` pasándole el contenido de todos los specs de feature generados.
