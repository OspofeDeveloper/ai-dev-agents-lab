...
| `sdd-features-index.py` | `test_sdd_features_index.py` | genera `_features.md`; idempotente byte-identico; `--check` 0/2; `--stdout`; layout subcarpeta y plano legacy; los 5 estados canonicos (incl. RETIRADA, que gana al readiness); readiness autoritativo **salvo un `PENDIENTE_GENERACIÓN` sobre spec que existe** (matriz estancada, D-089: el disco gana y avisa); las copias de CANON_STATES no divergen |
| `sdd-fanout-check.py` | `test_sdd_fanout_check.py` | tanda en un unico mensaje -> OK; formas `1 + 1` y `1 + (N-1)` -> exit 2 con el hueco en segundos; los 3 falsos positivos que hay que NO marcar (delegaciones secuenciales al mismo agente con skills distintas, relanzado tras turno humano, las N filas de un mismo `message.id`); `--json`; directorio -> sesion mas reciente; `.jsonl` truncado |
