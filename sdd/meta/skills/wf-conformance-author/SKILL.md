...
description: "Audita la cobertura de conformance de una skill (o de una fase entera) contra el catalogo de Casos de Uso y escribe los escenarios CU-N.x que falten en formato verbatim, actualizando la matriz del ROADMAP. Espejo de autoria de wf-sdd-audit."
...
- **`--cu <cu-NN>`** (opcional): fuerza el fichero `cu-NN` donde escribir (p. ej. `--cu cu-03`). Si se omite, elige el CU temáticamente correcto (`kb-sdd-conformance` Regla 8).
...
```
(en modo fase, el alcance son las filas de esa sección del ROADMAP; procesa una skill a la vez para no diluir tu propio contexto).
...
2. **Los `cu-NN` que la tocan** — los IDs `CU-N` que aparecen en esa columna (o el `--cu` forzado). Lee esos ficheros completos:
   ...
   ```
3. **Los IDs ya usados** en esos ficheros, para continuar la secuencia sin colisión:
   ```bash
...
Tras escribir, comprueba que no quedaron IDs duplicados (gate de `kb-sdd-conformance` Regla 1):
```bash
grep -hoE 'CU-[0-9]+\.[a-z]+' conformance/casos-de-uso/cu-*.md | sort | uniq -d
```
Cualquier salida = ID duplicado introducido → renombra antes de cerrar, y dilo en el informe.
