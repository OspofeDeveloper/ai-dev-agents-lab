...
4. **Recalibración, no maquillaje.** Si un CU marca **FALLO** una conducta que, mirada de cerca, es **razonable**, el defecto suele estar en el **contrato del CU**, no en el agente. Entonces: corrige el CU **y** registra la decisión (`D-NNN` en `DECISIONS.md`) que lo justifica — **nunca** relajes el criterio en silencio para forzar el verde. Es el análogo de la regla `DIVERGENTE` de QA (un test que falla contra un CA no se ajusta, se investiga). *Aprendido: D-024 recalibró CU-2.d de "siempre pregunta" a ponderado-por-riesgo tras ver que la conducta del agente era correcta.*
   > **La proporción es el dato, no la anécdota.** Un incumplimiento aislado es un despiste del agente; **uno sistemático es un defecto del contrato**. Si la conducta correcta incumple la regla en *todas* las corridas, la regla ha dejado de discriminar: ya no separa lo bueno de lo malo, solo genera FALLOS falsos que **tapan los hallazgos reales** de la misma pasada. Antes de endurecer una regla que nadie cumple, comprueba si lo que prohíbe es de verdad lo dañino. *Aprendido en CU-3.a pasada 4 ([[D-047]]): el contrato exigía un flag que viajó en **0 de 10** delegaciones, mientras las 10 esperas eran impecables por otra vía igual de legítima. El criterio estaba escrito por exclusión ("ninguna otra cosa cuenta") y excluía una ruta de la plataforma que no habíamos previsto.*
...
7. **El banco incluye el ciclo de vida de los agentes, no solo los ficheros.** Durante una pasada **no toques cómo corren los agentes**: no mandes uno a segundo plano para leer mejor sus logs, no lo interrumpas, no lo relances. Cambia lo que se mide y **no siempre deja rastro**. *Aprendido en CU-3.a: la pasada 2 se backgroundeó a mano para capturar logs y estuvo dando un diagnóstico falso durante semanas — `Cmd+B` no aparece en los transcripts (cero coincidencias en 20 ficheros de sesión), así que "background porque lo decidió el harness" y "background porque lo pulsó el humano" son **indistinguibles a posteriori**.* Y no hace falta: los logs de subagente ya están en disco, en `~/.claude/projects/<proyecto>/<sesión>/subagents/` (`.jsonl` con los parámetros exactos de cada tool call, `.meta.json` con `spawnDepth` y `parentAgentId`). Si lo que quieres medir es un parámetro o una relación padre-hijo, **ya está escrito**: no repitas la pasada, léelo.

9. **Un cambio de modelo caduca lo conductual, no lo determinista — y una pasada de humo puede romper, nunca sellar.** Cambiar el modelo de los agentes es una **frontera**: al otro lado, todo veredicto **conductual** anterior pasa a ser histórico (punto 3). Lo **determinista** no se toca — script, `grep`, la suite: lo cubre CI y le da igual el modelo (punto 1). Ese filtro es el que hace el re-chequeo asequible: de los escenarios de un CU sellado, solo hay que volver a pasar los que dependen de **juicio del agente** (gates que deben disparar, guardrails, arbitrajes, no-autoaprobación), no los de topología, instalación o recuento.
   > **Qué compra una pasada única, dicho con precisión.** No sella: el punto 2 sigue pidiendo ≥3. Lo que da es la **asimetría** que ese mismo punto ya reconoce — no puede confirmar que la conducta sigue bien, pero **sí puede demostrar que se rompió**, y un FALLO con una corrida ya basta para parar. Regístralo como un estado propio (`SELLADO (<modelo viejo>) · HUMO OK (<modelo nuevo>)`), **nunca como un `✅` a secas**: un sello que viene de una medición caducada tiene que verse que lo es.
   >
   > **Y no cruces dos fronteras a la vez.** *Aprendido en CU-3.a pasada 6: se apagó fork mode ([[D-050]]) y se bumpearon los agentes a los modelos 5 en la misma pasada. Salió bien, pero el salto de gaps —5/2 contra una banda de 11-16— no se puede atribuir a ninguno de los dos con confianza. Mueve una variable por pasada, o pierdes la capacidad de explicar lo que midas.*
...
> Esta regla es la SSoT que referencia la plantilla de PR (`docs/MEJORAS_FUTURAS.md` O-6): la sección "Impacto conductual" de un PR se rellena aplicando los puntos 1-3, 9 y 10.
