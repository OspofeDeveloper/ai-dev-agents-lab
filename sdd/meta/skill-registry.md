...
| wf-stack-create | Crea el esqueleto de un nuevo overlay de stack (tech/<stack>) conforme a kb-sdd-stack-overlay-contract: directorios, install.sh, wf-<stack>-init (detect/config… | true | meta/skills/wf-stack-create/SKILL.md |
...
| Skill | Descripción | Invocable | Path |
|---|---|---|---|
| kb-kmm-project-state-protocol | SSoT del protocolo de precondicion de contexto tecnico de los agentes KMM: leer kmm_project_state.md si existe, bloquear si no, excepcion unica para kmm-explor… | false | tech/kmm/skills/kb-kmm-project-state-protocol/SKILL.md |
...
| wf-kmm-init | Init especialista del stack KMM: produce kmm_project_state.md con el estado tecnico (targets, arquitectura, DI, networking, auth, storage, recursos, navegacion… | true | tech/kmm/skills/wf-kmm-init/SKILL.md |
...
| Skill | Descripción | Invocable | Path |
|---|---|---|---|
| kb-cmp-resources | Recursos compartidos en Compose Multiplatform con compose-resources (Res.*) a nivel planificación: qué es el sistema, estructura de carpetas, semántica de loca… | false | tech/kmm/skills/plan/kb-cmp-resources/SKILL.md |
...
| kb-kmm-app-layer | Capa app en proyectos KMM: composition root, navegación, composición entre features, coordinación de estado efímero de pantalla y decisiones dependientes de br… | false | tech/kmm/skills/plan/kb-kmm-app-layer/SKILL.md |
...
| kb-kmm-brands | Brands en proyectos KMM: identidad de producto por marca, catálogo de brands, valores base por marca, assets/naming/bundle id base y separación entre diferenci… | false | tech/kmm/skills/plan/kb-kmm-brands/SKILL.md |
...
| kb-kmm-core-layer | Capa core en proyectos KMM: dominio compartido, infraestructura transversal, criterios para subir responsabilidades desde features y reglas para evitar que cor… | false | tech/kmm/skills/plan/kb-kmm-core-layer/SKILL.md |
...
| kb-kmm-feature-clean-architecture | Arquitectura interna de una feature KMM: capas presentation/domain/data, ubicación de contratos e implementaciones, dependencias permitidas y criterios para su… | false | tech/kmm/skills/plan/kb-kmm-feature-clean-architecture/SKILL.md |
| kb-kmm-gradle-modules | Estructura de módulos Gradle en KMM como decisión de plan: materializar capas lógicas (app/core/feature) en módulos físicos (:composeApp, :core:*, :feature:*),… | false | tech/kmm/skills/plan/kb-kmm-gradle-modules/SKILL.md |
...
| kb-kmm-network-contracts | Contratos de networking en proyectos KMM: errores de red, resultados tipados, límites entre servicios remotos y repositorios, y reglas estables independientes… | false | tech/kmm/skills/plan/kb-kmm-network-contracts/SKILL.md |
| kb-kmm-offline-strategy | Estrategia de caché y offline-first en KMM como decisión de plan: cuándo cachear, Room como Single Source of Truth, patrón stale-while-revalidate, flujo sin co… | false | tech/kmm/skills/plan/kb-kmm-offline-strategy/SKILL.md |
| kb-kmm-secrets-cicd | Mecánica de transporte de secretos en CI/CD para KMM: llevar API keys, client secrets y signing keys desde local y CI hasta Android (BuildConfig) e iOS (xcconf… | false | tech/kmm/skills/plan/kb-kmm-secrets-cicd/SKILL.md |
...
| kb-plan-cmp-ui | Capa presentation en Compose Multiplatform (CMP) a nivel planificación: estructura del módulo UI compartida en commonMain, cuándo usar expect/actual de UI y re… | false | tech/kmm/skills/plan/kb-plan-cmp-ui/SKILL.md |
...
| kb-plan-kmm-datastore-preferences | Preferences DataStore en KMM como decisión arquitectónica de plan: cuándo usarlo, qué módulos se ven afectados, ownership core vs feature, separación storage/n… | false | tech/kmm/skills/plan/kb-plan-kmm-datastore-preferences/SKILL.md |
| kb-plan-kmm-navigation-viewmodel-events | Eventos y efectos de navegación desde ViewModel en KMM: patrón Intent/Events, Channel vs StateFlow, reglas de LaunchedEffect y separación entre lógica de negoc… | false | tech/kmm/skills/plan/kb-plan-kmm-navigation-viewmodel-events/SKILL.md |
...
| kb-plan-kmm-ui-text | Patrón UIText en Compose Multiplatform como decisión arquitectónica: qué problema resuelve y cuándo usarlo, separación ViewModel-UI para exponer texto traducib… | false | tech/kmm/skills/plan/kb-plan-kmm-ui-text/SKILL.md |
...
| kb-tasks-cmp-ui | Implementación de la capa presentation en proyectos Compose Multiplatform (CMP): entry points Android e iOS, setup de initKoin en iOS, ciclo de vida en commonM… | false | tech/kmm/skills/tasks/kb-tasks-cmp-ui/SKILL.md |
| kb-tasks-expert | Reglas para descomponer Planes técnicos KMM en Tasks atómicas delegables a agentes KMM: formato de task, granularidad, orden canónico de implementación, asigna… | false | tech/kmm/skills/tasks/kb-tasks-expert/SKILL.md |
...
| kb-tasks-kmm-integration-testing | Patrones de implementación de tests de integración KMM/CMP: árbol semántico Compose con composeTestRule, accesibilidad con useUnmergedTree, screenshot regressi… | false | tech/kmm/skills/tasks/kb-tasks-kmm-integration-testing/SKILL.md |
...
| kb-tasks-kmm-room | Implementación de Room en KMP: entidades y DAOs en commonMain, @Database con RoomDatabaseConstructor, builder por plataforma, BundledSQLiteDriver, KSP por targ… | false | tech/kmm/skills/tasks/kb-tasks-kmm-room/SKILL.md |
