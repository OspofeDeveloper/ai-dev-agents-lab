...
|---|---|---|---|---|---|---|---|---|---|
| wf-project-init | `--topology <a\|c\|s>` `--surfaces <…>` `--prd` `--design` `--stack` `--name` `--sdd-path` `--force` | orquestador en hilo principal (`AskUserQuestion`, sin fork); `sdd-init-detect.py` (detect/verify); `install.sh --no-claude-md`; hook SessionStart; `sdd-version.json` | CU-1.b/d/e/h/i/j/k/l/m/n/o/p · CU-10.d | ✅ | ✅ | ✅ | ✅ | COMPLETADO | Flags y enum inválido cubiertos por CU-1.p; entrevista interactiva validada a mano (los escenarios hook-only a/c/f/g son cobertura del hook de sesión, sin fila propia) |
