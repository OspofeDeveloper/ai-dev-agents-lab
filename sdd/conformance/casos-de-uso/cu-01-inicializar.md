...
## CU-1.u — La regla eager `sdd-routing.md` se instala topology-gated ([[D-022]])

**Precondición:** proyecto recién inicializado (o reinstalado con `wf-sdd-update`).
**Mecanismo:** `install.sh` `install_routing_rule` — ensambla `.claude/rules/sdd-routing.md` concatenando el `pipeline/<fase>/routing.md` de **cada fase instalada** (`has_phase`, scoped por fase), **sin** frontmatter `paths:` (carril eager, igual que `sdd-orchestration.md`). Contiene solo la **desambiguación + precondiciones/fronteras** que las `description` de los skills no cubren (p. ej. "crea specs → features-first, NO discover", patrón "fase X", elección de rigor, precondición "PRD revisable"); el rootmap-tabla de la regla de fase (lazy) se eliminó ([[D-023]]); nunca se replicó aquí. Distínguelo de CU-1.t: allí la regla (`sdd-orchestration.md`) es transversal e incondicional; aquí el contenido es **por-fase** — cada fase con `routing.md` contribuye solo si está instalada. Cerrado el roll-out de [[D-022]], las 5 fases (prd/spec/design/plan/tasks) aportan `routing.md`, así que en cualquier init real el fichero existe; el invariante verificable es el **scoping** (no la ausencia total, ya no alcanzable por instalación válida). Backstop determinista: `test_install_sh.py` (`test_routing_rule_is_scoped`, `_has_{prd,design,plan,tasks}_border`, `_has_spec_precondition`, `_is_eager_and_dual_audience`); este CU valida el resultado **en disco** en un repo real.
...
4. **El contrato de los `STOP_*` viaja en la regla** ([[D-075]]). Con `spec` instalada, la sección de esa fase tiene que nombrar el veredicto `STOP_*` y decir **quién lo presenta** (el hilo principal), no solo que el worker para.
   → **Esperado:** la sección `## Fase Spec` contiene `STOP_`, la frase de quién presenta el bloqueo, y la salvedad de los dos que **ningún `--allow-*` cierra** (mapa de capacidades brownfield y CAs `[INFERIDO]`: esperan confirmación, no permiso).
   → **Por qué se mide:** Design lo tenía escrito desde [[D-072]] y Spec no, pese a emitir **siete** `STOP_*` desde forks. La cadena `STOP_` no aparecía en **ninguna** regla eager: la convención que usan 10 skills de 5 fases no estaba nombrada en nada que el hilo principal cargue. Backstop: `test_routing_rule_carries_the_spec_fork_gate_paragraph`.
