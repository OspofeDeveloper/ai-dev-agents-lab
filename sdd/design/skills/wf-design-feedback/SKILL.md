...
description: Workflow SDD que captura feedback no estructurado de stakeholders (cliente, PM, dev, QA) y lo triajea en categorias accionables (cambio de brief, delta visual, ajuste de feature, fuera de scope). Permite que el ida-y-vuelta con stakeholders no rompa la trazabilidad del sistema. Activa en frases como "incorpora este feedback del cliente", "tengo comentarios del PM sobre el design", "el cliente dice que no le gusta el color de los botones, ¿que hacemos?", "procesa este feedback de stitch".
...
Sobrescribir el archivo de captura con esta estructura ampliada:
