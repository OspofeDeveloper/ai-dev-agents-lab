...
- ⚠ **`sdd-prd-apply.py`** (nuevo enforcement script): aplica `--confirm`/`--edit`/`--reject` sobre las asunciones (mapeo **posicional** N-ésima marca inline ↔ N-ésima entrada `[ASN-XXX]`, con precondición 1:1 y guarda fail-safe contra desalineación) y `--seal "<valor>"` para estampar `Aprobado por:`. Si vacía la sección de asunciones, la elimina entera. Exit 0/1/2.
