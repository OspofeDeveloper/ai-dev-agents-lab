...
- 🔴 **`DEPRECATION` tenía cero consumidores.** Estaba definida en `kb-product-change-governance`, `wf-prd-change` la clasificaba y el usuario la confirmaba en su gate — y ahí se acababa. Medido sobre un fixture: el spec de una capacidad cancelada **está limpio** (nadie lo ha tocado), así que el índice lo daba `LISTA`, el gate de planificación lo dejaba pasar y el estado del proyecto lo ponía en «Siguiente foco» pidiendo que se planificara.
- ⚠ **Estado nuevo, `Estado: RETIRADO`,** con su traza `Retirada: CR-XXX — <razón> (<fecha>)`. Vive en la **cabecera del spec** porque `_features.md` es función pura de sus fuentes y se reescribe entera: un estado anotado ahí se pierde en la siguiente regeneración. Lo escribe solo `sdd-seal.py --retire`, que no exige condiciones de sellado —se retira igual un spec roto que uno sano— pero **sí exige el `CR-XXX`**: retirar sin traza es cancelar producto sin registro.
- ⚠ **Skill nueva `wf-spec-retire`**, en el hilo principal y con la forma de `wf-spec-validate`: main pregunta, `sdd-spec-auditor` diagnostica el impacto (shared models que quedan huérfanos, quién la declara dependencia, qué artefactos ya existen) y el script estampa. **Sin flag de override** — los `--allow-*` son para lo que no puede preguntar ([[DECISIONS D-026]]); aquí el gate **es** el mecanismo. Si la feature tiene release, el gate lo dice: el código sigue entregado y darlo de baja aquí no lo retira.
- 🔴 **El bloqueo es real, no documental.** `sdd-gate-check.py` deniega planificar, generar tareas y ejecutarlas, resolviendo el `Spec origen` del plan y el `Plan origen` del tasks. **Asimetría deliberada con la enmienda**, que retiene solo las tasks del CA afectado: una baja cancela la feature entera, no queda subconjunto que ejecutar. Sin esto el plan seguía `VALIDADO` y las tareas seguían corriendo — el fallo justo donde importa.
- ⚠ **Quinto estado canónico `RETIRADA`** en las **11 copias** del vocabulario, con **backstop en tests** que compara las enumeraciones en prosa y la tupla de `sdd-project-status.py` contra `CANON_STATES` ([[DECISIONS D-069]]). Un estado que le falte a esa tupla no se pinta mal: hace **desaparecer** la feature del informe.
- ⚠ **La Regla 2 de gobernanza deja de solaparse consigo misma.** Decía *"añade/**elimina** capacidad"* en `SCOPE_CHANGE` y *"deja de ser vigente"* en `DEPRECATION`, y **ninguna de las dos estaba en la cadena de precedencia**. Ahora: mover a fase futura **no retira** (sigue comprometida); retirar es salir del producto; y la cadena queda completa con las cinco, encabezada por `DEPRECATION`.
- 🔴 **Colateral, mayor de lo diagnosticado:** `sdd-seal.py` casaba `### CA-003 [ELIMINADO en v1.4: …]` como un CA vigente. Se buscaba un síntoma —el plan debía "cubrirlo"— y había dos: **el spec tampoco se podía sellar**, porque a un tombstone se le exigía declarar HU padre. Aplicar la Regla 12, que este ecosistema sanciona como la forma correcta de eliminar, dejaba spec y plan insellables.
- ⚠ `wf-prd-change-cascade` deja de desmentirse: decía en dos sitios que ese paso *"nunca fue checkpoint humano"* y en un tercero que *"este es el primer checkpoint humano"*.
- **Aprendizaje:** una etiqueta sin consumidor no es una capacidad, es una promesa. `DEPRECATION` se clasificaba bien, el usuario la confirmaba, y no pasaba nada — y nadie lo notó porque el fallo no es ruidoso: desde abajo, una feature cancelada **no se distingue de una lista para empezar**.
...
- ⚠ **Las skills de auditoría dejan de declarar `Write`.** `sdd-spec-auditor` y `sdd-auditor` lo tienen en `disallowedTools`, así que ese `Write` no habilitaba nada — pero **conservan `Bash`**, y un `cat >` escribe igual. Medido: con el mismo frontmatter, dos auditores escribieron su informe con `cat >` y un tercero le pasó la escritura al hilo principal, que volcó el artefacto con un heredoc y lo firmó como suyo. `wf-spec-conflict`, `wf-spec-readiness`, `wf-prd-sync-impact` y `wf-sdd-audit` pasan a `[Read, Bash]` y **dicen en su cuerpo por qué vía se escribe**.
- **Regla de linter `AGENT-TOOL-CLASH` [blocking]:** `allowed-tools` que declara una tool prohibida por su `agent:`. Delta **4 → 0** sobre el árbol real (cazó una que un barrido a mano no vio).
- ⚠ **El read-only se escribe como norma en los dos agentes auditores.** *"Tienes `Write` y `Edit` prohibidos, pero conservas `Bash`: lo que te impide tocar lo que auditas es esta norma, no el candado."* Es el corolario de [[DECISIONS D-038]] en la dirección contraria — quitar una tool no quita la capacidad.
- ⚠ **`sdd-analysis-gaps.py` gana `--export-answers` / `--import-answers`**, y `wf-spec-analyze` los ejecuta alrededor de la sobrescritura. Regenerar el análisis destruía las respuestas ya dadas, que son decisiones de negocio de una persona — y un PRD que cambia **después** de responder los gaps no es raro: es el caso normal. **Empareja por ID *y* por título, y lo que no case exacto no se escribe**: el análisis no es reproducible (11/5 → 12/7 → 11/7 → 16/11 → 5/2 → 7/4 sobre el mismo PRD), así que un `P-XXX` puede ser otra pregunta en el documento nuevo. Exit 2 si queda algo por reconciliar.
- ⚠ **El changelog de producto registra toda versión del PRD, la bumpee quien la bumpee.** Un review que corrige prosa al cerrar asunciones también mueve `version`: el ciclo medido fue de 1.4 a 1.5 y lo dejó solo dentro del expediente de un CR. El índice se quedó en 1.4, mintiendo sobre el dato por el que se consulta.
- ⚠ **El análisis declara la versión del PRD en su cabecera** y **`wf-spec-discover` la verifica** antes de construir el mapa de features. Se puede parar sin coste **porque el rescate de respuestas ya existe**: antes, avisar de un análisis obsoleto obligaba a elegir entre datos viejos o perder las decisiones del usuario.
- **La frontera de [[DECISIONS D-048]] se afila: recortar no convierte prosa en metadato.** Un `grep` de campos redactados pasado por `cut -c1-400` sigue siendo prosa. El criterio es **qué** extraes, no cuánto. Y el recuento no se hace dos veces: medido, un `grep -oE` de marcadores devolvió **10** donde el script decía **4**.
- **Los agentes de `meta/` suben a modelo 5** (`sdd-auditor` → sonnet-5; `sdd-author` y `sdd-conformance` → opus-5). El bump de 0.83.x solo había tocado `pipeline/*/agents/`. Los 10 de `tech/kmm/` siguen en 4.x a propósito: tienen su propia conformance.
- **El informe de `wf-prd-sync-impact` se declara efímero** — lleva cabecera `Vigente para: PRD v<X.Y>` y la skill ofrece **no volcarlo** cuando el único derivado que señala se va a regenerar acto seguido.
...
- ⚠ **`sdd-features-index.py` cruza la frontera de topología.** Busca el discovery en `--discovery <path>` → dentro del directorio dado → los demás directorios de `artifacts` de `.sdd/project-init.json`, empezando por `prd`. En topología `authoring` (`artifacts.prd` != `artifacts.spec`) el discovery vive **con el PRD**, porque se deriva de él y se llama `<basename_prd>_discovery.md`. Un discovery externo **no renombra el índice**.
- ⚠ **Se acaba el silencio.** Si hay specs y no aparece discovery, el aviso viaja **dentro del artefacto** (`> ⚠ SIN DISCOVERY: …`) y por `stderr`. La política conservadora se mantiene; lo que se va es que un índice ciego fuera indistinguible de uno completo. **Los índices generados con la versión anterior en topología `authoring` están incompletos: regenéralos.**
- ⚠ **`wf-spec-readiness` lee los informes de conflictos de los dos sitios** y en ambos layouts. El fan-out del Paso 7 de `wf-spec-features-first` lanza **un auditor por spec** y cada uno escribe junto al suyo; buscar solo en la raíz dejaba fuera **todos**.
- **Regla de autoría nueva:** quien escribe y quien lee buscan en el mismo sitio, y eso se comprueba **en cada topología y en cada layout**. Cuando no aparezca lo esperado, decirlo en voz alta — en el artefacto, no solo en la consola.

**La sincronía de la delegación deja de ser prosa: hook `PreToolUse` sobre `Agent`** — [[DECISIONS D-048]]. Tercera iteración sobre el mismo punto: [[DECISIONS D-043]] lo prescribió, [[DECISIONS D-045]] lo hizo posible llevando el orquestador a main, [[DECISIONS D-047]] lo reencuadró… y el flag seguía viajando **0 de 10 veces**. Cuando una propiedad importa, hay que hacerla mecánica.

- ⚠ **Nuevo `sdd-agent-sync.py`** (matcher `Agent`, hermano de `sdd-gate-check.py`): deniega la delegación a un agente **del ecosistema** sin `run_in_background: false`, con un motivo que le dice al modelo qué hacer — repetir la misma llamada con el flag, emitir el fan-out en un único mensaje, y **no** cambiar de estrategia. `wf-sdd-update` lo instala y añade el matcher al `settings.json` existente sin tocar lo demás.
- **Alcance cerrado a los agentes SDD.** `general-purpose`, `Explore` y los agentes propios del usuario **no se tocan**: el ecosistema se instala en repos ajenos. Escotilla `SDD_ALLOW_ASYNC_AGENTS=1`. Y falla hacia lo de siempre — cualquier entrada inesperada permite y sale 0, nunca bloquea.
- ⚠ **La frontera del orquestador se escribe sobre el efecto, no sobre la tool** (cierra `O-8`). "No hagas `Read` del artefacto" se cumplía al pie de la letra con `cat`. Ahora: una extracción es legítima si es **determinista + delimitada + de metadatos** (IDs, veredictos, recuentos); traer prosa para interpretarla es analizar contenido, que es el trabajo del delegado. La Regla de oro de `wf-spec-features-first` nombra ya **todos** los artefactos, `_features.md` incluido.

**Esperar es que te entreguen el informe, no en qué turno llega** — [[DECISIONS D-047]]. En la misma pasada, `run_in_background: false` viajó en **0 de 10** delegaciones… y las 10 esperas fueron impecables: el orquestador cedió el turno y consumió el informe de la **notificación de fin**, sin sondear nada. Diez de diez no es un despiste: nuestro criterio excluía por redacción una ruta legítima de la plataforma. Una regla que la conducta correcta incumple siempre no gobierna nada — solo genera fallos falsos.

- ⚠ **El criterio de espera nombra las dos vías:** el `tool_result` de la propia llamada `Agent`, o la notificación de fin del agente. Lo que separa esperar de deducir **no es el turno, es quién trae el dato**: sigue prohibido —igual de duro— mirar si el fichero existe o deja de crecer, fiarse del veredicto de un script sobre él, o leer el `.output`/log del harness.
- ⚠ **El flag pasa de "siempre o es fallo" a "obligatorio donde hay barrera".** Un fan-out de N delegados seguido de un paso que lee lo de **todos** (regenerar el índice, auditar el conjunto) solo se sostiene con **las N llamadas en un único mensaje + el flag**. Medido: los tres `sdd-spec-writer` salieron en tres mensajes; hoy es inocuo porque van asíncronos, pero **serializaría el fan-out** en cuanto el flag surta efecto — un requisito enmascarado por el incumplimiento de otro.
- ⚠ **Los auditores de conflictos pueden contradecirse, y el orquestador no arbitra.** Sobre el mismo par, dos dijeron `SIN_CONFLICTOS` y uno levantó un conflicto ALTA. Un `SIN_CONFLICTOS` significa *"no lo vi desde aquí"*, no *"no existe"*: **nunca se cierra por mayoría**. La divergencia se pasa al readiness como encargo explícito, y allí se resuelve citando los specs y dejando constancia.
...
- ⚠ **Toda delegación por la tool `Agent` lleva ahora `run_in_background: false`** en las 7 skills: `wf-spec-features-first`, `wf-task-run`, `wf-bug`, `wf-prd-change`, `wf-prd-review`, `wf-kmm-init` (+ `wf-prd-create`, que ya lo tenía). Desde Claude Code v2.1.198 los subagentes corren en **background por defecto**: sin el flag el orquestador no recibe el resultado. En `wf-task-run` era lo más caro — delega la implementación y acto seguido valida el DoD y commitea.
- ⚠ **Los cinco puntos de delegación de `wf-spec-features-first` usan el mismo mecanismo**, con el `subagent_type` que el propio sub-workflow ya declara: analyze/discover → `sdd-spec-explorer`, fast-track → `sdd-spec-writer`, conflict/readiness → `sdd-spec-auditor`. El flag **no** rompe el fan-out: N llamadas en un único mensaje siguen corriendo a la vez, solo que el mensaje no vuelve hasta que todas terminan.
- ⚠ **Cláusula anti-sondeo:** esperar es esperar el retorno de la tool. Nada de `ls`/`find` en bucle sobre el directorio de artefactos, `Monitor` sobre el fichero que va a escribirse, ni relanzar un segundo agente "por si acaso" — eso es un race de doble escritura sobre el mismo artefacto.
- **Regla nueva de lint `AGENT-DISPATCH-UNSYNCED` (blocking)** en `sdd-structural-lint.py`, solo sobre `wf-*` (una `kb-*` que *describa* el patrón no es una prescripción). Delta medido: **6 findings antes del fix, 0 después**.
- **Regla canónica de autoría** en `kb-sdd-creation-guide` + su plantilla de frontmatter, junto a la excepción `fork ⊥ AskUserQuestion` — para que la lección viaje a las skills nuevas en vez de quedarse en la que la sufrió.
- **`wf-prd-change-cascade` no se toca:** delega por `Skill` declarado y está sellado en conformance sin sondeo observado. La regla es *por skill* (un mecanismo, síncrono), no "todo el mundo a `Agent`".

## 0.78.0 — 2026-08-02

**Las respuestas del `_analysis.md` tienen dueño, y "¿quedan gaps críticos?" deja de ser juicio** — [[DECISIONS D-042]]. Medido en CU-3.a: el hilo principal ofreció *"me las dictas y **yo las anoto** en el análisis"*, una vía que el contrato no contemplaba — los tres sitios que hablan del tema dicen siempre que escribe el usuario. Y al investigarlo apareció el defecto mayor: la detección de `[CRÍTICO]` sin responder era **lectura de prosa** gobernando dos ramas duras.

- ⚠ **Nuevo `sdd-analysis-gaps.py`** (se instala en `.sdd/scripts/`). `--check` da veredicto (`CRITICAL_OPEN` / `CRITICAL_ANSWERED` / `VACUOUS`), conteos e IDs; `--answer P-XXX "texto"` sustituye el `_(pendiente)_` de ese gap y solo esa línea.
- ⚠ **`wf-spec-features-first` Paso 2.5 y `wf-spec-gap-resolve` consultan el script antes de decidir.** El `--check` corre **delante** de la rama que gobierna (`--allow-open-critical-gaps`), con backstop que compara **posiciones** — la lección de [[DECISIONS D-037]]: cuando el texto y la posición se contradicen, gana la posición.
- ⚠ **Tabla cerrada de quién escribe** en `kb-gap-conventions`: responder un `[P-XXX]` es del **usuario** (edita el fichero, coste cero); si lo **dicta**, main lo aplica por script; el contenido de una respuesta no lo pone nadie más. **Main nunca hace `Read`/`Edit`/`Write` del `_analysis.md`**, aunque tenga las tools.
- **El mensaje de cierre deja de ser genérico:** path exacto, los IDs `[CRÍTICO]` cada uno con su pregunta en una línea, y qué se sustituye. Ya no hay que bucear entre 11 gaps para descubrir cuáles bloquean.
- **Guarda `VACUOUS`:** si el documento no tiene bloques de gap parseables, exit 2 en vez de "0 críticos abiertos" — un verificador que aprueba lo que no ha entendido es peor que no tenerlo.
- **`AskUserQuestion` se descartó** para recoger las respuestas: son prosa libre sobre 11 gaps y la tool va de opciones, con 4 preguntas por llamada. Sería la fricción del gate de asunciones del PRD, multiplicada.

## 0.77.0 — 2026-08-02

**Ningún agente declara `memory:`: el estado vive en los artefactos** — [[DECISIONS D-041]]. 24 agentes (pipeline, meta y overlay KMM) llevaban `memory: project` heredado de la plantilla de `kb-sdd-creation-guide`, sin que ninguna decisión lo hubiera elegido. Medido en CU-3.a: `sdd-spec-explorer` se escribió los 11 gaps con sus IDs, los defaults propuestos y un `How to apply:` que **instruye a las pasadas futuras** — un segundo contrato, no versionado, compitiendo con el SKILL.

- ⚠ **`memory:` fuera de los 24 agentes y de la plantilla de agente.** La plantilla es la pieza que importa: es de donde hereda su frontmatter cada agente nuevo.
- ⚠ **Los proyectos ya inicializados conservan sus `.claude/agent-memory/<agente>/` escritos.** El ecosistema **no los borra**: son datos en tu repo. Si quieres que los agentes dejen de leerlos, elimínalos tú (`rm -rf .claude/agent-memory`).
- **El motivo no es la higiene del banco de pruebas** sino la premisa del ecosistema: el artefacto es la SSoT. La memoria de agente sobrevive fuera del artefacto — ningún gate la comprueba, ningún script la ve, ninguna regeneración la invalida. Si un agente parece necesitarla, lo que necesita es leer su artefacto o recibir el dato en el brief.
- **No se toca `autoMemoryEnabled` del proyecto:** apagaría también la memoria de tus propias conversaciones en tu repo, y `CLAUDE_CODE_DISABLE_AUTO_MEMORY` es una env var que no viaja en git. El campo `memory:` del agente es la superficie que sí es del ecosistema.
- Nueva regla de lint `AGENT-MEMORY-DECLARED` (blocking) en `sdd-structural-lint.py`, que cubre agentes **y** plantilla, más backstop de instalación. Efecto lateral buscado: las pasadas de conformance vuelven a ser independientes entre sí.
...
- ⚠ **`sdd-prd-apply.py`** (nuevo enforcement script): aplica `--confirm`/`--edit`/`--reject` sobre las asunciones (empareja marca inline anónima ↔ entrada `[ASN-XXX]` por **texto exacto** y, para las parafraseadas, por **máximo solapamiento de contenido**; precondición 1:1 + guarda fail-safe) y `--seal "<valor>"` para estampar `Aprobado por:`. Si vacía la sección de asunciones, la elimina entera. Exit 0/1/2.
- ⚠ **`wf-prd-review`**: `allowed-tools` pierde `Read` y `Write` — el hilo principal ya **no carga ni reescribe** el PRD; las ediciones (Paso 5.5) y el sello (Paso 6) se aplican vía `sdd-prd-apply.py` por `Bash`, desde las decisiones que recoge el gate. "Quién edita" no cambia (lo corre el orquestador, no un agente); solo el mecanismo.
- **Alcance:** la prosa de brief que dependía de una asunción rechazada (nivel b, líneas sin marca) sigue resolviéndose en el gate con el usuario, no la mecaniza el script (límite heredado de Q2a).
- **Tests:** `test_sdd_prd_apply.py` (nuevo, incl. integración: aplicar + sellar → `READY` en `sdd-prd-ready.py`), `test_prd_review_applies_edits_via_script` + `sdd-prd-apply.py` en `ENFORCEMENT_SCRIPTS`.
- **CU:** sin CU nueva (script determinista → test black-box); a re-validar por cambio de mecanismo **CU-2.e (probe D)** y **CU-2.f**.
