...
description: "Orquestador del cascade de cambio de producto: tras un cambio de PRD encadena en un solo comando la secuencia que hoy se ejecuta a mano (change → sync-impact → spec-sync analyze/apply → conflict → readiness → design-sync → reporte de plan/tasks stale). Corre automáticamente todo el pegamento mecánico y read-only Y aplica sin parar los deltas de spec inequívocos (severidad minor + acción delta); para SOLO en los checkpoints humanos reales (aprobación del cambio, features con cambio no trivial, decisiones de Design, revalidación de plan). --review-before-apply restaura la parada conservadora antes de cualquier apply. Profundidad adaptativa: solo entra en Design si existe DESIGN.md, solo reporta planes/tasks si existen, y degrada con gracia si un workflow aguas abajo no está instalado."
