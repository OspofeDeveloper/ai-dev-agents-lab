...
- **Repo de diseño aparte** (D-011): `design_role = null`, **NO se instala la fase `design`** aquí (el diseño se resuelve del repo `design` en solo lectura). Captura:
  ...
  - **Guard de cobertura de target (D-011/D-012):** comprueba que el repo de diseño **cubra la familia que consume este repo** — lee `<DESIGN_SOURCE>/.sdd/project-init.json` → `target_platforms`; si la familia de los `DESIGN_TARGETS` del consumer (el **primer token** de cada etiqueta: `mobile-android`→`mobile`, `web`→`web`) **no** está entre ellas, **avisa** (no bloquea): «el repo de diseño cubre `[<target_platforms del design_source>]` pero este repo consume `[<familias del consumer>]`; no hay bundles para tu plataforma → al resolver heredarás la **base agnóstica** (posiblemente sesgada a otra superficie). Amplía el repo de diseño a esa familia (`wf-design-delta`) o revisa el `design_source`». **Colócalo en el mismo punto de decisión** (la confirmación del repo de diseño y/o el resumen pre-instalar), no lo entierres en un comentario suelto. Si `<DESIGN_SOURCE>` no declara `target_platforms` legible, **no afirmes un gap**. Tras escribir el `project-init.json`, el espejo determinista es `sdd-source-drift.py check` (`design_ssot.design_target_coverage.gap` + `uncovered_families`). No reescribas los `design_targets` del repo de diseño ni fabriques bundles para la familia ausente.
