...
- **Un `deny` con `[SDD-SYNC]` no es un bloqueo: es un parámetro que falta ([[D-048]]).** El hook `sdd-agent-sync.py` lo emite cuando la llamada sale sin el flag. La respuesta correcta es **repetir la misma llamada** —mismo `subagent_type`, mismo `prompt`— añadiendo `run_in_background: false`. **No** cambies de mecanismo (nada de `Skill`), **no** hagas el trabajo tú, y **no** le digas al usuario que el entorno no te deja delegar. Si el mismo deny se repite tres veces, para y díselo: puede apagarlo con `SDD_ALLOW_ASYNC_AGENTS=1`, y eso significa que el contrato dejó de ser cumplible.
