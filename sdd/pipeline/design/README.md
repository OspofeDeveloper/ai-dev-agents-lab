...
| **Escala y multiplataforma (D-011)** | Además del greenfield sin diseñador: equipos con **diseño dedicado** y **escala multi-repo** — repo de diseño SSoT propio (topología `design`) compartido por varias superficies/productos, con **divergencia multi-plataforma nativa** (Material vs HIG, phone/tablet) como **base ⊕ override per-view** (`kb-design-feature-artifacts` R8) + capa de componente por plataforma en `DESIGN.md` (`kb-design-system-contract` R11), consumida por los repos de código en **solo lectura** (`design_source`, `sdd-design-resolve.py`; drift cross-repo con `sdd-source-drift.py`). Aditivo: el flujo greenfield no cambia |
...
| **Qué no cubre** | Integración Figma (import de variables / export de Figma Tokens) sigue **fuera de alcance** (ROADMAP 7.6, 2026-06-10); destinos Stitch (mobile) y `web-generic`. **D-011 (2026-06-18):** el fit ya se expandió a equipos con diseño dedicado / multi-repo, así que la condición de 7.6 para reconsiderar Figma queda satisfecha — pero reabrir Figma es una decisión de producto **separada y aún no tomada**; hasta entonces, fuera. Un equipo con diseñador que ya produce specs visuales puede además escribir `DESIGN.md` a mano respetando el contrato y auditarlo con `wf-design-validate` |
