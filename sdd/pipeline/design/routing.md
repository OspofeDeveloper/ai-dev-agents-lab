...
**Los gates de esta fase se presentan en el momento, no se dictan desde un fork ([[D-072]]).** Las skills de Design que **conversan** con el usuario —cerrar el brief recorriendo el árbol de decisión, capturar inspiración visual, curar las referencias del research, fijar la hipótesis de un A/B y registrar su resultado— **corren en el hilo principal** y sostienen sus preguntas con `AskUserQuestion`; se reconocen en su frontmatter, sin `context: fork`. Las demás son **workers**: cuando topan con algo que exige decisión humana **paran y devuelven el bloqueo** con veredicto `STOP_*`, y el override `--allow-*` lo arma el usuario eligiendo ([[D-026]]/[[D-064]]). Cuatro de esas conversaciones estaban escritas dentro de forks, donde **no eran ejecutables**: el riesgo medido no era quedarse a medias, era rellenar por cuenta propia la dirección visual del producto.
