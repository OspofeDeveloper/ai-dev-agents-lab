...
**`Depende de:` no es para reenunciados.** Enlaza asunciones **distintas** donde una no tiene sentido sin la otra (p. ej. «presupuesto por categoría» ← «las categorías son gestionables»). **No** lo uses para atar dos formulaciones de la **misma** decisión (p. ej. «opera con moneda única» ↔ «multi-divisa fuera de alcance»): eso no son dos asunciones dependientes, es **una** reenunciada. Si te descubres poniendo `Depende de:` entre una regla transversal y su exclusión-complemento, es la señal de que sobra una — colápsala a una sola (ver la deriva de *reenunciado* en la correspondencia 1:1).
...
- **No repitas la misma decisión en varias secciones (reenunciado).** Una decisión inferida se enuncia **una vez** en su lugar canónico, con **una** marca y **una** entrada. El fallo tiene dos formas:
  - *Reenunciado desnudo:* reafirmarla en otra sección (p. ej. `## Reglas de Negocio Transversales`) como segunda marca `[ASUNCIÓN]` **sin entrada propia** → **huérfana** (rompe el 1:1).
  - *Reenunciado vestido (más sutil — el 1:1 no lo pilla):* darle a cada formulación **su propia** marca **y** entrada. El invariante `sdd-prd-ready.py` pasa (marcas = entradas), pero el usuario acaba **decidiendo dos veces la misma decisión**, y pueden quedar incoherentes (confirmar «moneda única» y rechazar «multi-divisa fuera») — algo que el review no puede resolver. **Ejemplo canónico:** «la aplicación opera con una única moneda» (regla transversal) y «conversión/gestión multi-divisa: fuera de alcance» (exclusión) son **la misma** decisión de producto — la exclusión es el complemento lógico de la regla. Va **una** sola `[ASN-XXX]`, en su forma más informativa (la regla/capacidad positiva subsume a su exclusión-complemento); **nunca** dos atadas con `Depende de:`.
  - Si la regla transversal (o la exclusión) es una decisión inferida **distinta** —no el complemento de otra ya marcada— entonces sí lleva su propia marca **y** su propia entrada.
