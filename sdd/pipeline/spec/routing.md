...
**Guardrails de `wf-spec-features-first`** (el workflow se detiene; el hilo principal comunica y espera decisión):
...
**Elección del rigor (standard / ligero), por feature.** No se fija en el init: es elección deliberada por feature al crear el spec. Como `wf-spec-fast-track`/`wf-spec-features-first` corren en `context: fork` (no pueden usar `AskUserQuestion`, ver [[D-002]]), **el hilo principal ofrece la elección ANTES de invocar**:
