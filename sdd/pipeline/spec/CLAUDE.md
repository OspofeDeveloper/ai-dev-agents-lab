...
**Todo** artefacto que produce esta fase **lo lee el usuario**, no solo el pipeline: los que generas (`_analysis.md`, `_discovery.md`, `_code_discovery.md`, `_spec.md`), los que derivas de ellos (`_delta_analysis.md`, `_sync_requirements.md`) y los informes (`_conflict_report.md`, `_readiness_report.md`, `_sync_report.md`, `_features.md`). La regla es del **tipo de contenido**, no de una lista de ficheros: si lo escribes en un `.md` del proyecto, aplica. Ahí dentro rige la misma frontera que en el chat (SSoT: `sdd-orchestration.md`, "Comunicación con el usuario"):

- **Estado y procedencia se quedan como están**: marcadores (`[CRÍTICO]`, `[INCOMPLETO]`, `[INFERIDO]`), veredictos (`LISTO_PARA_SPECS`, `PENDIENTE_GENERACIÓN`), IDs (`P-XXX`, `F-00X`, `CF-F001-01`) y cabeceras de procedencia (`Generado por: wf-spec-discover`) son **contrato**: los parsean los scripts y los gates.
...
- **Tampoco se nombra la `kb-*` que sostiene un veredicto** ([[D-088]], [[D-091]]). Es la otra mitad de la misma clase y se escapa por otro sitio: no al recomendar un paso, sino al **justificar** —*"Regla 4 de `kb-conflict-expert`"*, *"contemplado como aceptable en `kb-prd-expert`"*—. La regla se cita **por lo que dice** (*"el mismo modelo con comportamientos distintos en dos CAs de features diferentes"*): así el veredicto se sostiene solo, y quien lo lee no necesita saber cómo está construido el sistema que se lo cuenta.
